...
Aggregate1<-function(SimulatedData){
  ...
  Average = {}
  Average$Date = headObj$SWMMTimes
  Average$Sim = Simulated$Q
  Average$Obs = CalibrationData[,2]
...
# #   i=1
# #   k=1
# #   while(i <=(length(Aggregate$Date1))){
# #     if (NumHours1[i+1]-NumHours1[i]!=0 & is.na(NumHours1[i+1]-NumHours1[i])==FALSE){
# #       Average$Date[k] = Aggregate$Date1[i+1]
# #       k=k+1
# #     }
# #     if (is.na(NumHours1[i+1]-NumHours1[i])==TRUE){
# #       Average$Date[k] = Aggregate$Date1[i-3]+ 3600
# #       k=k+1
# #     }
# #     
# #     i=i+1
...
CalculateEvent<-function(Average){
  #wkRain = loadWorkbook("Rainfall.xlsx") 
  Rainfall = readWorksheet(wkRain, sheet="Rainfall")# load XLConnect package 
  #wkSimObsDate = loadWorkbook("C:\\Users\\Alamdari\\Research\\R-SWMM Test Files\\WriteSimulatedObserved.xlsx")
  SimObsDate = data.frame(Average) 
  A = as.character.Date(Rainfall[,1])
  m = str_sub(A,12,19)
...
#         
  ...
  Dataframe = data.frame(EventsNew)
  Dataframe = Dataframe[-c(17,29,32,33),]
...
PerfomrmStatistic1<-function(Average){
  require(hydroGOF)
  sim2 = data.frame(Average$Sim)
  obs2 = data.frame(Average$Obs)
...
Objectivefunction<-function(SWMMOptFile,x,OutFile,swmm,Timeseries,StatParameters){
  ...
  Q = getSWMMTimeSeriesData(headObj,iType = 1,nameInOutputFile = "Outfall",vIndex = 4)
  ...
  ggof(Average$Sim, Average$Obs, na.rm = TRUE, pt.style = "ts", ftype = "o", stype="default", 
       gof.leg = TRUE,  digits=2, 
       gofs=c("me","RMSE","PBIAS","NSE","d","R2"),xlab = "Time", ylab=c("Q[cfs]"))
  SummaryStatistics1 = PerfomrmStatistic1(Average)
